 ...
 * {@link MetadataResolver} to retrieve expire-aware refreshing
 * {@link AssertingPartyMetadata} instances.
 *
...
public final class OpenSamlAssertingPartyMetadataRepository implements AssertingPartyMetadataRepository {
	...
	public Iterator<AssertingPartyMetadata> iterator() {
		...
		Iterator<EntityDescriptor> descriptors = resolve(all).iterator();
		return new Iterator<>() {
			@Override
	...
	public AssertingPartyMetadata findByEntityId(String entityId) {
		...
		EntityDescriptor descriptor = resolve(byEntityId).iterator().next();
		return OpenSamlAssertingPartyDetails.withEntityDescriptor(descriptor).build();
	}

	private Iterable<EntityDescriptor> resolve(CriteriaSet criteria) {
		try {
			return this.metadataResolver.resolve(criteria);
		}
	...
	/**
	 * Use this {@code metadataLocation} to retrieve SAML 2.0 Asserting Party (IDP)
	 * metadata.
	 *
	 * <p>
	...
	public static MetadataLocationRepositoryBuilder withMetadataLocation(String metadataLocation) {
		return new MetadataLocationRepositoryBuilder(metadataLocation);
	}
	...
	public static final class MetadataLocationRepositoryBuilder {
		...
		private MetadataLocationRepositoryBuilder(String metadataLocation) {
