	...
	private Consumer<Collection<Saml2Error>> verifySignature(Saml2LogoutRequest request, LogoutRequest logoutRequest,
		...
		for (Saml2X509Credential credential : credentials) {
			if (credential.getEntityId() == null) {
				credential.setEntityId(details.getEntityId());
			}
		}
		OpenSamlOperations.VerificationConfigurer verify = this.saml.withVerificationKeys(credentials);
		return (errors) -> {
			if (logoutRequest.isSigned()) {
				errors.addAll(verify.verify(logoutRequest));
			}
			else {
				OpenSamlOperations.VerificationConfigurer.RedirectParameters params = new OpenSamlOperations.VerificationConfigurer.RedirectParameters(
						request.getParameters(), request.getParametersQuery(), logoutRequest);
