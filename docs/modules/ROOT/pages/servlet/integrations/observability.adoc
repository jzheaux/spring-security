...
----
@Bean
ObservationObjectPostProcessor<FilterChainDecorator> noSpringSecurityRequestObservationInfrastructure() {
...
----
@Bean
fun noSpringSecurityRequestObservationInfrastructure(): ObservationObjectPostProcessor<FilterChainDecorator> {
	return ObservationObjectPostProcessor.noop()
}
----
======
...
And the following makes so that all Spring Security-managed ``AuthorizationManager``s are attached with a different `ObservationConvention`:
...
----
@Bean
ObservationObjectPostProcessor<AuthorizationManager<?>> customAuthorizationObservationConvention() {
	return new ObservationObjectPostProcessor<>() {
		@Override
		<O extends AuthorizationManager<?>> O postProcess(ObservationRegistry registry, O manager) {
			ObservationAuthorizationManager<?> observed =
					new ObservationAuthorizationManager<>(registry, manager);
...
----
@Bean
fun customAuthorizationObservationConvention(): ObservationObjectPostProcessor<AuthorizationManager<*>> {
	return object : ObservationObjectPostProcessor() {
		override fun <O : AuthorizationManager<*>> postProcess(registry: ObservationRegistry?, manager: O): O {
			val observed = ObservationAuthorizationManager(registry, manager)
			observed.setObservationConvention(myObservationConvention)
			return observed
		}
	}
