...
Instead of using xref:servlet/authorization/authorize-requests.adoc[`authorizeRequests`], use `authorizeHttpRequests`, like so:
...
However, {security-api-url}org/springframework/security/web/access/expression/WebExpressionAuthorizationManager.html[`WebExpressionAuthorizationManager`] is available to help migrate legacy SpEL.
