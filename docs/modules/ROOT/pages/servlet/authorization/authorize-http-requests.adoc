...
==== Supplying the Servlet Path as a `@Bean`

Specifying the servlet path for every URI can be cumbersome.
To remove this repetitive boilerplate, you can publish a `ServletRequestMatcherBuilders#servletPath` as a `@Bean` and Spring Security will use it for all URIs.
