...
ObservationPredicate noSpringSecurityObservations() {
	return SecurityObservationPredicate.noObservations();
}
----
...
fun noSpringSecurityObservations(): ObservationPredicate {
	return SecurityObservationPredicate()
}
----
======
...
For example, the `SecurityObservationPredicate` bean excludes the filter chain observations by default.
So, you can also do:
...
----
@Bean
SecurityObservationPredicate noSpringSecurityObservations() {
...
----
@Bean
fun noSpringSecurityObservations(): SecurityObservationPredicate {
	return SecurityObservationPredicate.withDefaults().build()
}
----
======
...
@Bean
SecurityObservationPredicate noSpringSecurityObservations() {
	return SecurityObservationPredicate.withDefaults()
...
[TIP]
====
Note that in doing this, Spring Security will still wrap all of its observable components in `ObservationXXX` implementations; it's simply the case that Micrometer will not report these observations.
====

=== Customizing Observation Wiring

You can customize how observations are attached to security components, you can publish an `ObservationObjectPostProcessor`.
The type for the `ObservationObjectPostProcessor` is the type of security component you want to configure for observations.

For example, the following makes so that the filter chain is not configured for observations:
...
----
@Bean
ObservationObjectPostProcessor<WebFilterChainDecorator> noSpringSecurityRequestObservationInfrastructure() {
...
----
@Bean
fun noSpringSecurityRequestObservationInfrastructure(): ObservationObjectPostProcessor<WebFilterChainDecorator> {
	return ObservationObjectPostProcessor.noop()
}
----
======
...
[TIP]
=====
The above is handy if you know you never want to use the filter chain observations and also don't want the extra stack trace elements for each wrapped security filter.
=====

And the following makes so that all Spring Security-managed ``ReactiveAuthorizationManager``s are attached with a different `ObservationConvention`:
...
----
@Bean
ObservationObjectPostProcessor<ReactiveAuthorizationManager<?>> customAuthorizationObservationConvention() {
	return new ObservationObjectPostProcessor<>() {
		@Override
		<O extends ReactiveAuthorizationManager<*>> O postProcess(ObservationRegistry registry, O manager) {
			ObservationReactiveAuthorizationManager<?> observed =
					new ObservationReactiveAuthorizationManager<>(registry, manager);
			observed.setObservationConvention(myObservationConvention);
			return observed;
		}
	};
}
...
----
@Bean
fun customAuthorizationObservationConvention(): ObservationObjectPostProcessor<ReactiveAuthorizationManager<*>> {
	return object : ObservationObjectPostProcessor() {
		override fun <O : ReactiveAuthorizationManager<*>> postProcess(registry: ObservationRegistry?, manager: O): O {
			val observed = ObservationReactiveAuthorizationManager(registry, manager)
			observed.setObservationConvention(myObservationConvention)
			return observed
		}
	}
...
[NOTE]
=====
Even though you can replace these anonymous classes with a lambda expression, it is important to leave them as anonymous classes so that the generic typing is preserved at runtime.
This allows for Spring to correctly resolve which `ObservationObjectPostProcessor` to use for which Security component.
=====
