...
CSRF protection is enabled by default for Reactive applications.
...
[NOTE]
====
If you are using Spring MVC `<form:form>` tag or http://www.thymeleaf.org/whatsnew21.html#reqdata[Thymeleaf 2.1+] and are using `@EnableWebFluxSecurity`, the `CsrfToken` is automatically included for you (using the `CsrfRequestDataValueProcessor`).
====
...
You can configure `CookieCsrfTokenRepository` for a reactive application:
...
[[csrf-timeouts]]
==== Timeouts
One issue is that the expected CSRF token is stored in the HttpSession, so as soon as the HttpSession expires your configured `ServerAccessDeniedHandler` will receive a `CsrfException`.
If you are using the default `ServerAccessDeniedHandler`, the browser will get an HTTP 403 and display a poor error message.
...
Alternatively, specifying a custom `ServerAccessDeniedHandler` allows you to process the `CsrfException` any way you like.
For an example of how to customize the `ServerAccessDeniedHandler`, refer to the provided links for both <<nsa-access-denied-handler,xml>> and https://github.com/spring-projects/spring-security/blob/3.2.0.RC1/config/src/test/groovy/org/springframework/security/config/annotation/web/configurers/NamespaceHttpAccessDeniedHandlerTests.groovy#L64[Java configuration].

Finally, the application can be configured to use <<csrf-cookie,CookieServerCsrfTokenRepository>> which will not expire.
...
For example, the following Java Configuration will perform logout when the URL /logout is requested with any HTTP method:
...
//TODO: How does reactive handle multipart today?
...
For example, you can provide a custom ServerCsrfTokenRepository to override the way in which the `CsrfToken` is stored.

You can also specify a custom ServerWebExchangeMatcher to determine which requests are protected by CSRF (i.e. perhaps you don't care if log out is exploited).
...
Refer to the `ServerHttpSecurity#CsrfSpec` javadoc for details on how to make these customizations for a WebFlux application.
