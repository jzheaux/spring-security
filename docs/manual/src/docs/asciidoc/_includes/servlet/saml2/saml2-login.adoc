...
To customize validation, you can implement your own `AuthenticationManager`.
At this point, the validation is minimal, so you may be able to first delegate to the default `AuthenticationManager` like so:

[source,java]
----
@Component
public class MyOpenSamlLogoutRequestAuthenticationManager implements AuthenticationManager {
	private final AuthenticationManager delegate = new OpenSamlLogoutRequestAuthenticationManager();
...
Then, you can supply your custom `AuthenticationManager` in the DSL as follows:
...
http
    .saml2Logout((saml2) -> saml2
        .logoutRequest((request) -> request
            .authenticationManager(myOpenSamlLogoutRequestAuthenticationManager)
        )
    );
----
...
----
@Component
public class MyOpenSamlLogoutResponseAuthenticationManager implements AuthenticationManager {
	private final AuthenticationManager delegate = new OpenSamlLogoutResponseAuthenticationManager();
...
http
    .saml2Logout((saml2) -> saml2
        .logoutResponse((response) -> response
            .authenticationManager(myOpenSamlLogoutResponseAuthenticationManager)
        )
    );
----
