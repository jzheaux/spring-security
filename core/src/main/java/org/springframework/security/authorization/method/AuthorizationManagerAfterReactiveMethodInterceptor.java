...
public final class AuthorizationManagerAfterReactiveMethodInterceptor
	...
	public Object invoke(MethodInvocation invocation) throws Throwable {
		...
		Function<Object, Object> toInvoke = (result) ->
				this.authorizationManager.verify(authentication, new MethodInvocationResult(invocation, result))
						.thenReturn(result);
		// @formatter:on
		if (Mono.class.isAssignableFrom(returnType)) {
			return ((Mono<?>) ReactiveMethodInvocationUtils.proceed(invocation))
					.flatMap((result) -> this.authorizationManager
							.verify(authentication, new MethodInvocationResult(invocation, result)).thenReturn(result));
		}
		if (Flux.class.isAssignableFrom(returnType)) {
			return ((Flux<?>) ReactiveMethodInvocationUtils.proceed(invocation))
					.flatMap((result) -> this.authorizationManager
							.verify(authentication, new MethodInvocationResult(invocation, result)).thenReturn(result));
		}
		if (hasFlowReturnType) {
			...
			if (isSuspendingFunction) {
				response = Flux
						...
						.flatMap((result) -> this.authorizationManager
								.verify(authentication, new MethodInvocationResult(invocation, result))
								.thenReturn(result));
			}
			else {
				...
				response = Flux.from(adapter.toPublisher(flowProceed(invocation)))
						.flatMap((result) -> this.authorizationManager
								.verify(authentication, new MethodInvocationResult(invocation, result))
								.thenReturn(result));
			}
		...
		return Flux.from(ReactiveMethodInvocationUtils.proceed(invocation))
				.flatMap((result) -> this.authorizationManager
						.verify(authentication, new MethodInvocationResult(invocation, result)).thenReturn(result));
	}
