...
public final class ExceptionHandlingConfigurer<H extends HttpSecurityBuilder<H>> extends
	...
	private AccessDeniedHandler createDefaultDeniedHandler(H http) {
		if (defaultDeniedHandlerMappings.isEmpty()) {
			return new AccessDeniedHandlerImpl();
		}
		if (defaultDeniedHandlerMappings.size() == 1) {
			return defaultDeniedHandlerMappings.values().iterator().next();
		}
		return new RequestMatcherDelegatingAccessDeniedHandler(
				defaultDeniedHandlerMappings,
