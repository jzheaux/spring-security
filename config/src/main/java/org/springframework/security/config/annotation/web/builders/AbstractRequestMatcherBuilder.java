...
public abstract class AbstractRequestMatcherBuilder implements RequestMatcherBuilder {
	...
	public RequestMatcher[] matchers(HttpMethod method, String... patterns) {
		...
		for (int index = 0; index < patterns.length; index++) {
			matchers[index] = matcher(method, patterns[index]);
		}
		return matchers;
	}
	...
	public RequestMatcher[] matchers(String... patterns) {
		RequestMatcher[] matchers = new RequestMatcher[patterns.length];
		for (int index = 0; index < patterns.length; index++) {
			matchers[index] = matcher(patterns[index]);
		}
		return matchers;
	}

	abstract RequestMatcher matcher(HttpMethod method, String pattern);

	abstract RequestMatcher matcher(String pattern);
