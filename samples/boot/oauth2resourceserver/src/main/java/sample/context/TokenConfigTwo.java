...
public class TokenConfigTwo {
	@Bean
	NimbusJwtEncoder jwsEncoder(RSAKey key) {
	...
	@Bean
	Consumer<JwsHeaderMutator<?>> jwsCustomizer() {
		return (headers) -> headers.criticalHeader("exp", Instant.now());
	}

	@Bean
	Consumer<JwtClaimMutator<?>> claimsCustomizer() { // might want to override at invocation time
	...
	private static class TenantJwkSource implements JWKSource<SecurityContext> {
		...
		public List<JWK> get(JWKSelector jwkSelector, SecurityContext context) throws KeySourceException {
			if (context instanceof TenantSecurityContext) {
				...
				return this.sources.get(0).get(jwkSelector, (TenantSecurityContext) context);
			}
