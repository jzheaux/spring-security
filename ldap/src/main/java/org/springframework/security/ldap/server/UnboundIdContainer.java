...
public class UnboundIdContainer
	...
	public void start() {
		...
		try {
			...
			RDN rdn = Objects.requireNonNull(dn.getRDN(), "defaultPartitionSuffix cannot be the empty DN");
	...
	private void importLdif(InMemoryDirectoryServer directoryServer) {
		if (StringUtils.hasText(this.ldif)) {
			Assert.notNull(this.context, "context cannot be null if ldif as a value");
			try {
