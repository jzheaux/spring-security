 ...
 * @since 5.8
 */
public aspect PostAuthorizeAspect implements InitializingBean {

		/**
		 * Matches the execution of any method with a PostAuthorize annotation.
		 */
		private pointcut executionOfAnnotatedMethod() :
				execution(* *(..)) && @annotation(PostAuthorize);

		private ExceptionWrappingMethodInterceptor securityInterceptor;

		Object around(): executionOfAnnotatedMethod() {
			if (this.securityInterceptor == null) {
				return proceed();
			}
			MethodInvocation invocation = new MethodInvocationAdapter(thisJoinPoint, () -> proceed());
		...
		public void setSecurityInterceptor(MethodInterceptor securityInterceptor) {
			this.securityInterceptor = new ExceptionWrappingMethodInterceptor(securityInterceptor);
		}
